...
- **CTA's 'L' trains don't have live positions yet, only buses do** — Bus Tracker identifies a bus by route + scheduled departure time, which is enough to match it back to a real scheduled trip. Train Tracker identifies a train by run number instead, and there's no reliable bridge from a run number back to a specific scheduled trip in CTA's static GTFS — solving that needs more research, not just a wiring change, so 'L' trains stay schedule-only for now.
