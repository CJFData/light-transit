...
- **NYC Subway's realtime is split across 8 separate MTA feeds by line group, not one combined feed** — unlike LIRR/Metro-North's single URL. `MultiGtfsFeed` (the same mechanism Bustang uses to merge into RTD Denver) now covers this case too: with no static feed of its own (`feedUrl` left null), it's just another realtime source layered onto the primary agency's own already-ingested schedule, unioned in with no id-prefixing since the trip_ids already match directly — versus Bustang's case, a genuinely separate static+realtime feed that does need prefixing to avoid colliding with RTD's own ids.
